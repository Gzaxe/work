...
class CoulombPotentialParticle:
    ...
    def equations_of_motion(self, t, y):
        """哈密顿方程"""
        x, p = y
        dxdt = p / self.m + 28.9176 * np.sin(1.51859 *10**(18) * t)
        dpdt = self.force(x)
...
if __name__ == "__main__":
    ...
    # 设置初始条件
    x0 = 3.8894971293263245*10**(-11)  # 初始位置
    p0 = -1.788530745804815*10**(-21 ) # 初始动量
    
    # 运行模拟
    t, x, p, energy = simulator.simulate(x0, p0, t_start=1.24125*10**(-17), t_max=10**(-16), num_points=2000)
...
# 保存为CSV
df.to_csv(save_dir + 'fig3-a-phase2-origin.csv', index=False)
